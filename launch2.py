...
if __name__ == '__main__':
  ...
  try:
    proc_cvt_multicopter = subprocess.run(f'xacro ./models/multicopters/xacro/multicopter_0.xacro  > ./models/multicopters/urdf/multicopter_0.urdf', shell=True)
  except subprocess.CalledProcessError as e:
    print(e)
    exit()
  ...
  for i in range(ground_station_antenna_num):
    ...
    try:
      proc_cvt_gs = subprocess.run(f'xacro ./models/ground_station_antennas/xacro/{ground_station_antenna}.xacro  > ./models/ground_station_antennas/urdf/{ground_station_antenna}.urdf', shell=True)
    except subprocess.CalledProcessError as e:
      print(e)
      exit()
  ...
  try:
    proc_set_multicopter = subprocess.run(f'ign service -s /world/multicopter/create \
  ...
  # proc_hvr = subprocess.Popen('exec ign topic -t "/multicopter_0/gazebo/command/twist" -m ignition.msgs.Twist -p "linear:{x:0 y:0 z:0} angular {z: 0}"', shell=True)
  ...
  for i in range(ground_station_antenna_num):
    ground_station_antenna = 'ground_station_antenna_' + str(i)
    try:
      proc_set_gb = subprocess.run(f'ign service -s /world/multicopter/create \
                    --reqtype ignition.msgs.EntityFactory \
                    --reptype ignition.msgs.Boolean \
                    --timeout 5000 \
